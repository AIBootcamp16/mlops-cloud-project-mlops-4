...
def _list_prevday_files(prev_day_str_compact: str) -> List[str]:
    pat = re.compile(
        rf"^{re.escape(PROJECT)}_{prev_day_str_compact}T\d{{2}}_.+\.parquet$"
    )
...
def _merge_parquet_files(input_paths: List[str], output_path: str):
    ...
    writer = pq.ParquetWriter(
        output_path, schema=schema, compression=PARQUET_COMPRESSION
    )
    try:
...
def _save_local_as_s3_path(
    merged_local: str, s3_key: str, scope_label: str
) -> str | None:
    """
    ...
    try:
        ...
        log.exception(
            f"[{scope_label}] 로컬 저장 실패: {merged_local} → {local_dest}: {e}"
        )
        return None
...
def merge_and_upload_previous_hour(prev_hour_dt: datetime):
    ...
    merged_local = os.path.join(OUT_DIR, f"{PROJECT}_{base_compact}.parquet")
    ...
    s3_key = f"{S3_PREFIX}/{day_dash}/{os.path.basename(merged_local)}"
    ...
    if ENABLE_S3_UPLOAD and _s3:
        try:
            ...
            log.exception(
                f"[HOUR] 업로드 실패: {merged_local} → s3://{S3_BUCKET}/{s3_key}: {e}"
            )
            # 실패 시 폴백으로 진행
